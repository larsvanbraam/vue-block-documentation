...
exports.getLocalSettings = function(settingsPath = '.') {
  if (this.hasLocalSettings(settingsPath)) {
    const fileContent = fs.readFileSync(path.join(settingsPath, settingsFile), {
      encoding: 'utf-8',
    });
    ...
    if (settings.variables) {
      ...
      settings.variables = settings.variables.map(variable =>
        _.merge({}, defaultVariable, variable),
      );
    }

 ...
 */
function parseBlockDirectory(blockDirectory, settings, subDirectory) {
  const fileName = blockDirectory + 'Data.js';
  const subDirectoryPath = subDirectory ? subDirectory + '/' : '';
  const sourcePath = path.resolve(`${settings.input}/${subDirectoryPath}${blockDirectory}`);
  const sourceFile = `${sourcePath}/${fileName}`;
  const tempPath = `${path.resolve(
    Config.OUTPUT_TEMP_FOLDER,
    settings.input,
...
module.exports = function generate(settings) {
  ...
  if (fs.existsSync(blockFolder)) {
    ...
    return (
      createDirectory(Config.OUTPUT_TEMP_FOLDER)
        .then(() =>
          sequentialPromises(
            blockDirectories.map(directory => () => {
              const directoryPath = path.resolve(`${settings.input}/${directory}`);
              const subDirectories = getDirectories(directoryPath);
              if (subDirectories.length) {
                return sequentialPromises(
                  subDirectories.map(subDirectory => () =>
                    parseBlockDirectory(subDirectory, settings, directory),
                  ),
                );
              } else {
                return parseBlockDirectory(directory, settings);
              }
            }),
          ),
        )
